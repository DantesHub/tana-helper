...
import pinecone
from ..dependencies import PineconeRequest, PineconeNode, get_embedding, TANA_NAMESPACE, TANA_TYPE
...
async def upsert(request: Request, req: PineconeRequest):
    async with lock:
        ...
        logger.info(
            f'DONE txid={request.headers["x-request-id"]} time={formatted_process_time}')
        return None
...
def get_tana_nodes_for_query(req: PineconeRequest, send_text: Optional[bool] = False):
    ...
    if not send_text:
        ...
        best = filter(threshold_function, query_response.matches)
        docs = [{'sources': '[[^'+match.id+']]',
                 'answer': match.metadata['text']} for match in best]
        return docs
