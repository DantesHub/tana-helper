...
TANA_ENVIRONMENT = os.environ.get(
    "PINECONE_ENVIRONMENT") or "us-west4-gcp-free"
...
def get_embedding(req: PineconeRequest):
    ...
    embedding = openai.Embedding.create(
        input=req.context, model=req.embedding_model)
...
class LineTimer:
    ...
    def __exit__(self, exc_type, exc_value, traceback):
        ...
        logger.info('Code block' + self.name +
...
def tana_to_json(tana_format):
    ...
    for line in tana_format.split('\n'):
        ...
        if in_code_block:
            ...
            if '```' in line and line[0:3] == '```':
                ...
                if current['is_field']:
                    ...
                    newobj = {'name': code_block, 'is_field': False,
        ...
        newobj = {'name': line, 'is_field': is_field,
    ...
    def hoist_field(node, parent):
        ...
        if 'children' in node:
            ...
            if value and children:
                raise Exception(
                    'Field with both value and children is not supported')
            if children:
                value = children
